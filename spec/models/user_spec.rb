...
describe User do
  ...
  before do
    @user = User.new(name: "User", real_name: "Example", email: "user@example.com",
  ...
  describe "when real name is too long" do
    before { @user.real_name = "a" * 76 }
    it { should_not be_valid }
  end

  describe "when real name is too short" do
    before { @user.real_name = "a" }
    it { should_not be_valid }
  end
